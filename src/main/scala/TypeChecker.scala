	...
	def typerecurse(AST: Tree, parent: Tree, symbol: HashMap[String, Type]): (Tree, Type, HashMap[String, Type]) = {
		AST match {
			...
			case binopNode(op, l, r) =>
				...
				val (ret, typ) = (op, ltyp, rtyp) match {
					...
					case ("*", arrayType(t), intType) => (arrayMultNode(left.elements.first, right), arrayType(t))
					case ("*", intType, arrayType(t)) => (arrayMultNode(right.elements.first, left), arrayType(t))
			...
			case accessNode(arrayId, idx) =>
				...
				val typ = arrayId match {
					case arrayNode(elements) => typerecurse(arrayNode(elements))._2.typ
					case wordNode(id) => symbol(id).typ
				}
				//TODO: recurse idx
